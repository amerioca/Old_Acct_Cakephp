...
class UserAuthComponent extends Component {
	...
	function beforeFilter(&$c) {
		...
		$permissionFree=array('users/login', 'users/logout', 'users/register',
            'users/userVerification', 'users/forgotPassword', 'users/activatePassword',
            'pages/display', 'users/accessDenied', 'users/emailVerification',
            'entrances/cron', 'discounts/cron', 'orders/start', 'orders/submit');
     ...
     * @return string
     */
    public function isSuperEmployee() {
        $groupId = $this->Session->read('UserAuth.User.user_group_id');
        if($groupId==4) {
            return true;
        }
	...
	private function __useCookieToken() {
		...
		$user=false; //var_dump($token);
		// Extract the duration appendix from the token
        if(is_array($token)){
		if(strpos(current($token), ":") !==false) {
