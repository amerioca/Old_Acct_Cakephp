...
class UsersController extends UserMgmtAppController {
	...
	public function index() {
		...
		$users=$this->User->find('all', array('order'=>'User.id desc', 'conditions'=>'User.user_group_id>='.$this->UserAuth->getGroupId()));
		...
		if ($this->request -> isPost()) {
			...
			if($this->User->LoginValidate()) {
				...
				if (empty($user)) {
					...
					if (empty($user)) {
						$this->Session->setFlash(__('Incorrect Email/Phone number or Password'));
						return;
					}
				}
	...
	public function addUser() {
		$userGroups=$this->UserGroup->getGroups('UserGroup.id>='.$this->UserAuth->getGroupId());
		//debug($userGroups);
        $this->set('userGroups', $userGroups);
		if ($this->request -> isPost()) {
			$this->User->set($this->data);
